...
public class DecodeAndPlayVideo {
	...
	static BufferedImage bui = null;

	/**
	 * Takes a media container (file) as the first argument, opens it, opens up
	 * a Swing window and displays video frames with <i>roughly</i> the right
	 * timing.
	 * 
	 * @param args
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * IllegalArgumentException("must pass in a filename" +
		 * " as the first argument");
		 */

		// String filename = args[0];
		int count = 0;
		String filename = "h:\\parrot\\melt.mpg";
		...
		Socket socket_video_tcp = new Socket("192.168.1.1", 5555);
		if (container.open(socket_video_tcp.getInputStream(), null) < 0)
			throw new IllegalArgumentException("EEEkkkk");
			...
			if (packet.getStreamIndex() == videoStreamId) {
				...
				while (offset < packet.getSize()) {
					...
					if (picture.isComplete()) {
						...
						// bui = Utils.videoPictureToImage(newPic);

						// and display it on the Java Swing window
						if (count == 100) {
							dumpImageToFile(javaImage);
							count = 0;
						}
	...
	private static String dumpImageToFile(BufferedImage image) {
		try {
			String outputFilename = "c:\\temp\\temp.png";
