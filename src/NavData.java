...
import controller.ControlAlgorithm;
import controller.CtrlState;
import controller.Dimension;
import controller.FlyingState;
import controller.Mode;
import controller.NavDataTag;
import controller.Point;
import controller.VisionTag;
...
public class NavData {
	...
	private static List<VisionTag> parseVisionTags(ByteBuffer buf, int offset) {
		...
		for (int i = 0; i < nb_detected; i++) {
			...
			VisionTag vt = new VisionTag(VisionTag.VisionTagType.fromInt(type),
	...
	private static void parseState(int state) {
		...
		controlAlgorithm = (state & (1 << 3)) != 0 ? ControlAlgorithm.ANGULAR_SPEED_CONTROL
